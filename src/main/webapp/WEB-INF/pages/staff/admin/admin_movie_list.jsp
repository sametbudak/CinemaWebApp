...
<div class="container">
    ...
    <h2 class="text-center text-danger">Movies</h2>
    <br/>
    <table class="table table-bordered">
        ...
        <c:forEach var="movie" items="${listableMovieInfoList}">
            <tr>
                <td>${movie.id}</td>
                <td>${movie.title}</td>
                <td>${movie.status}</td>
                <td>
                    <c:url var="editUrl" value="/admin/get_edit_movie_page/${movie.id}"></c:url>
                    <a class="btn btn-default" href="${editUrl}"> Edit </a>
                </td>
                <td>
                    <c:choose>
                        <c:when test="${movie.deleteIsDisabled}">
                            <a class="btn btn-default" href="#" disabled> Delete </a>
                            ${movie.screeningMessage}
                            ${movie.ticketMessage}
                        </c:when>
                        <c:otherwise>
                            <c:url var="deleteUrl" value="/admin/delete_movie/page${activePage}/movie${movie.id}"></c:url>
    ...
    <div class="pagination text-center">
        ...
        <c:forEach begin="${fromPage}" end="${tillPage}" varStatus="loop">
            <li <c:if test="${loop.current eq activePage}">class="active"</c:if> ><a href="/admin/list_movies/${loop.current}">${loop.current+1}</a></li>
        </c:forEach>
